...
// ----------------------------------------------------
// [PARTIAL UPDATE] PATCH : 특정 데이터의 일부만 수정하기
// ----------------------------------------------------
const handlePatch = async (id) => {
  try {
    // PUT은 전체 데이터를 보내지만, PATCH는 바꿀 필드만 보냅니다.
    const patchPayload = { title: '🩹 PATCH로 타이틀만 수정' }

    const response = await axios.patch(`${BASE_URL}/${id}`, patchPayload)
    ...
    <ul class="item-list">
      <li v-for="item in items" :key="item.id" class="item-card">
        ...
        <div class="btn-group">
          ...
          <button @click="handlePatch(item.id)" class="btn-patch">PATCH (일부 수정)</button>
